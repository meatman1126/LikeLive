...
public interface BlogRepository extends JpaRepository<Blog, Long> {
    ...
    /**
     * 指定されたブログ情報を取得します。 関連エンティティの著者情報（author）を即時ロードします。 論理削除されていないデータのみを取得対象とします。
     *
     * @param id ブログID
     * @return 指定されたブログ情報
     */
    @EntityGraph(attributePaths = {
            "author" }) @NonNull @Query("SELECT b FROM Blog b WHERE b.id = :id AND b.isDeleted = false")
    Optional<Blog> findById(@NonNull Long id);

    /**
     * 指定されたユーザが作成した公開中のブログ記事を取得します。 取得結果はブログ作成日時の降順にソートされます。
     *
     * @param userId ユーザID
     * @return ブログ記事リスト
     */
    @Query("SELECT b FROM Blog b " + "WHERE b.author.id = :userId " + "AND b.status = 'PUBLISHED' " + // PUBLISHEDステータスを条件に追加
            "AND b.isDeleted = false " + "ORDER BY b.blogCreatedTime DESC")
    List<Blog> findPublishedBlogsByUserId(@Param("userId") Long userId);

    /**
     * 指定されたユーザが作成した下書きのブログ記事を取得します。 取得結果はブログ作成日時の降順にソートされます。
     *
     * @param userId ユーザID
     * @return ブログ記事リスト
     */
    @Query("SELECT b FROM Blog b " + "WHERE b.author.id = :userId " + "AND b.status = 'DRAFT' "
            + "AND b.isDeleted = false " + "ORDER BY b.blogCreatedTime DESC")
    List<Blog> findDraftBlogsByUserId(@Param("userId") Long userId);

    /**
     * 指定されたユーザの非公開状態のブログ記事を取得します。 取得結果はブログ作成日時の降順にソートされます。
     *
     * @param userId ユーザID
     * @return ブログ記事リスト
     */
    @Query("SELECT b FROM Blog b " + "WHERE b.author.id = :userId " + "AND b.status = 'ARCHIVED' "
            + "AND b.isDeleted = false " + "ORDER BY b.blogCreatedTime DESC")
    List<Blog> findArchiveBlogsByUserId(@Param("userId") Long userId);

    /**
     * キーワードに合致するブログ記事をページネーションとソートを用いて取得します。 ブログに関連するアーティスト名も検索対象に含め、ステータスがPUBLISHEDのもののみ取得します。
     * キーワードがnullまたは空文字の場合は空の結果を返します。
     *
     ...
     */
    @EntityGraph(attributePaths = {
            "author" }) @Query("SELECT DISTINCT b FROM Blog b LEFT JOIN b.blogArtists ba LEFT JOIN ba.artist a WHERE "
    ...
    /**
     * 指定したユーザがフォローしているユーザが作成したブログ記事を取得します（論理削除されていないデータ）。 取得結果はブログ作成日時の降順にソートされます。
     *
     * @param userId ユーザID
     * @return ブログ記事リスト
     */
    ...
    /**
     * 指定したユーザがフォローしていないユーザが作成したブログ記事を取得します（論理削除されていないデータ）。 取得結果はブログ作成日時の降順にソートされます。
     *
     * @param userId ユーザID
     * @return ブログ記事リスト
     */
     ...
     * @param blog 更新情報
     */
    @Modifying @Query("UPDATE Blog b SET " + "b.updatedBy = :#{#blog.updatedBy}, "
            + "b.blogUpdatedTime = :#{#blog.blogUpdatedTime}, " + "b.category = :#{#blog.category}, "
            + "b.content = :#{#blog.content}, " + "b.slug = :#{#blog.slug}, " + "b.status = :#{#blog.status}, "
            + "b.tags = :#{#blog.tags}, " + "b.thumbnailUrl = :#{#blog.thumbnailUrl}, " + "b.title = :#{#blog.title}, "
            + "b.setlist = :#{#blog.setlist} " + "WHERE b.id = :id")
     ...
     * @return 更新された行数
     */
    @Modifying @Query("UPDATE Blog b " + "SET b.status = 'ARCHIVED', b.updatedBy = :updatedBy "
            + "WHERE b.id = :blogId")
     ...
     */
    @Modifying @Query("UPDATE Blog b SET " + "b.isDeleted = true," + "b.updatedBy = :updatedBy " + "WHERE b.id = :id")
     ...
     * @param userId 更新ユーザID
     */
    @Modifying @Query("UPDATE Blog b SET " + "b.viewCount = :viewCount," + "b.updatedBy = :updatedBy "
            + "WHERE b.id = :id")
     ...
     * @param userId 更新ユーザID
     */
    @Modifying @Query("UPDATE Blog b SET " + "b.likeCount = :likeCount, " + "b.updatedBy = :updatedBy, "
            + "b.updatedAt = CURRENT_TIMESTAMP " + "WHERE b.id = :id")
     ...
     * @param userId 更新ユーザID
     */
    @Modifying @Query("UPDATE Blog b SET " + "b.commentCount = :commentCount," + "b.updatedBy = :updatedBy "
            + "WHERE b.id = :id")
    void updateCommentCount(@Param("id") Long id, @Param("commentCount") int commentCount,
            @Param("updatedBy") String userId);

    /**
     * ダッシュボードに表示する興味のあるブログ記事を取得します。 フォロー中のユーザの記事を優先的に取得します。
     *
    ...
    @Query("SELECT new com.example.bookstore.dto.repository.DashboardBlogRepositoryDto("
            ...
            + "b.blogCreatedTime) " + "FROM Blog b " + "WHERE b.author.id <> :userId "
    ...
    List<DashboardBlogRepositoryDto> findInterestBlogs(@Param("userId") Long userId,
            @Param("publishedStatus") BlogStatus publishedStatus, Pageable pageable);
